...
public class ProductoBean {
    ...
    public String registrarProducto(HttpServletRequest request) {
        ...
        if (connection != null) {
            try {
                StringBuilder query = new StringBuilder();
                query.append(" insert into producto (cod_cat,nom_pro,des_pro,costo_uc,costo_uv,stock,proveedor_cod_prov) ");
                query.append("values (?,?,?,?,?,?,?) ");
                ...
                insertProducto.setInt(1, codCat);
                insertProducto.setString(2, nombre);
                insertProducto.setString(3, descripcion);
                insertProducto.setDouble(4, cuc);
                insertProducto.setDouble(5, cuv);
                insertProducto.setInt(6, s);
                insertProducto.setInt(7, codProv);    
//ejecutando la consulta
    ...
    public List<String> getProductos(String nombre) {
        ...
        try {
            ...
            query.append(" select concat_ws(' ',p.cod_pro,p.nom_pro) ");
            query.append(" from producto p ");
    ...
    public ProductoCompra buscarProducto(String codProducto) {
        ...
        if (connection != null) {
            try {
                ...
                consulta.append(" select p.cod_pro, p.nom_pro, p.des_pro, p.costo_uc,p.costo_uv,p.stock ");
                ...
                if (resultado != null && resultado.next()) {
                    ...
                    productoCompra.setPrecioCompra(resultado.getFloat(4));
                    productoCompra.setPrecio(resultado.getFloat(5));
                    productoCompra.setStock(resultado.getInt(6));                   
